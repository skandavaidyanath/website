# Skanda Vaidyanath's résumé

The main LaTeX source is `mmayer.tex`. Build it locally with:

```sh
make build
```

To build the résumé and copy the resulting PDF to the website's
`static/files/resume.pdf` path, run:

```sh
make publish
```

The build uses [Tectonic](https://tectonic-typesetting.github.io/).

## Template provenance

This résumé uses AltaCV v1.1.5.
